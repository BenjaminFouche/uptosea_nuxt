...
<script setup>
// Plus aucun import n'est nécessaire ici !
// Nuxt auto-importe ref, computed, watch, onMounted, nextTick, useRoute, useRouter, et les stores Pinia.
import UiButton from '~/components/ui/UiButton.vue';
import BoatCard from '~/components/common/BoatCard.vue';
import SearchFiltersSection from '~/components/common/SearchFiltersSection.vue';
import AffinerRechercheSection from '~/components/common/AffinerRechercheSection.vue';
...
import ErreurApi from '~/components/common/ErreurApi.vue';
...
watch(
    ...
    () => {
      nextTick(() => {
        // Protection SSR : on s'assure qu'on est sur le navigateur avant de scroller
...
const applyFilterFromUrl = () => {
  ...
  if (slug) {
    ...
    if (categoryName) {
      boatsStore.filterBoatsByCategory(categoryName);
    } else {
      boatsStore.activeCategory = null;
    }
  }
  else {
    boatsStore.filterBoatsByCategory('Tous');
  }
};
...
const clearAllFilters = async () => {
  ...
  await navigateTo('/bateaux'); // Remplacement ici aussi
